...
class Nova(AsyncAgent):
    ...
    async def process(self, message: AgentMessage) -> TaskResult:
        ...
        self._log("💻 Processing code generation task...")
        ...
        if force_local_fallback:
            ...
            self._log(
                f"🧱 Streak fallback mode active (no-output streak={fallback_streak}), "
            ...
            if not self._is_output_usable(code_data):
                ...
                self._log("⚠️ Primary response missing deployable files, running strict repair pass...")
                repair_messages = [
                    {"role": "system", "content": self._get_system_prompt()},
            ...
            if not self._is_output_usable(code_data):
                used_local_scaffold = True
                self._log("🧱 Repair pass still empty, using deterministic local scaffold.")
                code_data = self._build_local_scaffold(context=context, iteration=iteration)
...
<head>
  ...
  <title>AutoDev Continuous Runtime</title>
  <link rel="stylesheet" href="styles.css" />
</head>
<body>
  <main class="app">
    <header class="hero">
      <p class="badge">AutoDev</p>
      <h1>Continuous Build</h1>
      <p class="subtitle">{escaped_goal}</p>
      <button id="themeToggle" class="btn" aria-label="Toggle theme">Toggle theme</button>
    </header>
    <section class="card-grid" aria-label="Highlights">
      <article class="card"><h2>Responsive</h2><p>Layout adapts from mobile to desktop.</p></article>
      <article class="card"><h2>Accessible</h2><p>Semantic structure and keyboard-safe controls.</p></article>
      <article class="card"><h2>Performant</h2><p>No framework overhead, just clean static assets.</p></article>
    </section>
  </main>
...
.app { max-width: 1000px; margin: 0 auto; padding: 2rem 1.25rem 3rem; }
.hero { display: grid; gap: 0.75rem; margin-bottom: 1.5rem; }
.badge { margin: 0; letter-spacing: 0.08em; font-weight: 700; color: var(--accent); }
...
"""
        return {
            ...
            "dependencies": [],
            "improvements": [
                "Upgrade visuals and content hierarchy based on latest design feedback.",
                "Expand interaction logic after baseline deploy is stable.",
            ],
        }
