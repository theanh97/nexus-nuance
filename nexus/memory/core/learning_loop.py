...
# NEW: Import Self-Improvement Engine
try:
    # Try from nexus.self_improvement first
    import sys
    from pathlib import Path
    nexus_path = Path(__file__).parent.parent.parent
    if str(nexus_path) not in sys.path:
        sys.path.insert(0, str(nexus_path.parent))
    from nexus.self_improvement import get_improvement_engine, run_self_improvement_cycle
...
class LearningLoop:
    ...
    INTERVALS = {
        ...
        "self_improvement": 300,  # Every 5 minutes - apply improvements
    }
    FOCUS_AREAS = [
        "reliability",
        "learning",
    ...
    def __init__(self, base_path: str = None):
        ...
        self.last_self_improvement = None  # NEW: Track self-improvement cycles
        self.self_improvement_interval = int(os.getenv("SELF_IMPROVEMENT_INTERVAL", "300"))  # 5 minutes
    ...
    def run_iteration(self) -> Dict:
        ...
        # 7.5. Self-Improvement Engine (when stagnation detected)
        if SELF_IMPROVEMENT_AVAILABLE and self.no_improvement_streak >= 50:
            try:
                improvement_engine = get_improvement_engine()
                cycle_result = improvement_engine.run_cycle(max_patches=2)
                results["self_improvement"] = {
                    "cycle_id": cycle_result.cycle_id,
                    "opportunities": cycle_result.opportunities_found,
                    "patches_applied": cycle_result.patches_successful,
                    "improvement_delta": cycle_result.improvement_delta,
                }
                results["actions"].append("self_improvement_engine")

                if cycle_result.patches_successful > 0:
                    self.no_improvement_streak = 0
                    self._append_rnd_note(
                        note_type="self_improvement_applied",
                        severity="info",
                        message=f"Self-improvement engine applied {cycle_result.patches_successful} patch(es)",
                        context={"cycle_id": cycle_result.cycle_id},
                    )
            except Exception as e:
                track_error("SYSTEM", "self_improvement_error", str(e), context={"iteration": self.iteration}, recoverable=True)
                results["errors"].append({"step": "self_improvement", "error": str(e)})
